...
class ZenCrowd():
    ...
    def e_step(self):
        ...
        for i in range(self.n_train):
            ...
            # mu_normal = (a * self.pi_normal[1]) / (a * self.pi_normal[1] + b * self.pi_normal[0])
            # print(a,b,self.pi_normal[1],self.pi_normal[0])
            # print(b, self.pi_normal[0])
            ...
            # if mu_normal == 0:
            #     mu_normal = 0.001
            # self.ground_truth_est_normal[i, 1] = np.log(mu)
            # self.ground_truth_est_normal[i, 0] = np.log(1.0 - mu)
            # print(final_weights_normal1)
            # print(final_weights_normal0)
    ...
    def m_step(self, epochs=1):
            ...
            for i in range(self.n_train):
                if self.answers[i, r] != -1:
                    ...
                    if self.answers[i, r] == 1:
                        ...
                        self.alpha[r] += self.ground_truth_est[i, 0]
                    else:
                        raise Exception()
            self.alpha[r] /= alpha_norm

        # print(self.answers_p)
        for r in range(self.num_annotators_p):
            alpha_norm_p = 1
            for i in range(self.n_train):
                if self.answers_p[i, r] != -1:
                    ...
                    if self.answers_p[i, r] == 1:
                        ...
                        self.alpha_p[r] += self.ground_truth_est[i, 0]
                    else:
                        raise Exception()
            # print(alpha_norm_p)
        ...
        for i in range(self.n_train):
            for j in range(self.num_annotators_p):
                self.weight_p[i][j][0] = np.log(self.alpha_p[j])
                if self.alpha_p[j] == 1:
                    self.alpha_p[j] = 0.999
                if self.alpha_p[j] == 0:
                    self.alpha_p[j] = 0.001
                self.weight_p[i][j][1] = np.log(1 - self.alpha_p[j])
                self.weight_p[i][j][2] = np.log(1 - self.alpha_p[j])
